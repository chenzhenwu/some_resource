...
def _prepare_directory(path):
    path=path.replace('\\',r'/')           #chenzhenwu add one line
    ...
    for scanner in scanners_data:
        ...
        for resource in resources:
            ...
            target = '{0}.json'.format(os.path.join(module_path,
                                                    resource['resource']).encode('cp936'))      #chenzhenwu add function .encode('cp936')
            _dir, _fname = os.path.split(target)
            parts = os.path.split(resource['path'].encode('cp936'))[0].split(os.sep)            #chenzhenwu add function .encode('cp936')
            print '*****', parts
            new_parts = []
